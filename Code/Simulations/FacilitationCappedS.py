import numpy as np
import pickle
class FacilitationCappedS:
    def __init__(self, dt, end, t_s, maxFreq, eyeStartParam, eyeStopParam, eyeResParam, nonLinearityFunction,
                 nonlinearityNoR):
        ...
        self.s_mat = np.zeros(
            (len(self.t_vect), self.neuronNum))  # txn 2d array for storing information from the simulations
    ...
    def RunFacilitationCappedS(self, predictionFunc, debug=True, startIdx=-1, dead=[], timeDead=100000000):
        ...
        # Sets the eye position to 25 or a set index of eye position
        if not startIdx == -1:
        ...
        eyePositions[0] = predictionFunc(self.s_mat[0, :])
        ...
        while tIdx < len(self.t_vect):
            ...
            #Calculate ds
            # t_s * ds/dt = -s + Prel * r * (1-s)
            decay = -self.s_mat[tIdx - 1]
            growth = np.multiply(P_rel[tIdx - 1], r_vect) * (1-self.s_mat[tIdx-1])
