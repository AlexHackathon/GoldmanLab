...
class FacilitationSim:
    ...
    def RunSimD(self, predictionFunc, debug=False, startIdx=-1, dead=[], timeDead = 10000000):
        ...
        if not startIdx == -1:
            #Set start eye position to specified index (startIdx)
            self.s_mat[0] = self.f(self.r_mat[:, startIdx])
        else:
            # Set the start eye position to 0
        ...
        #Vector that stores the firing rates throughout the simulation
        ...
        #Vector that stores Prel throughout the simulation
        P_rel = np.zeros((len(self.t_vect), self.neuronNum))
        P_rel[0] = np.array(self.f(self.r_mat[:, startIdx], self.P0, self.f_f, self.t_f))
