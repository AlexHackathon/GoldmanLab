...
abstract: "\\emph{A pdf copy of your assignment is due at 4:00pm PT Wednesday, Feb.
  28. Submission of the pdf will be through gradescope. Please put in the effort to
...
pander(ATable, caption="Table 1: Relative ratios under the Null hypothesis")
```
...
pander(ATable,
      col.names=Category, 
      digits=3,
      caption="Table 2: Observed vs. Expected values for Experiment 1")
```

From Table 2 above, we find that the peas in categories AB (round and yellow), aB (wrinkled and yellow), and AaB (yellow and both round and wrinkled) stand out somewhat in terms of their discrepancies between values observed in the sample and those expected under the Null hypothesis (i.e. their contributions to a Chi-squared test statistic).
...
pander(ATable, caption="Table 3: Relative ratios under Mendel's proposal")
```


Figure \ref{fig:expt2} below displays Mendel's observed counts for Experiment 2.
...
NullRatio2 <- c(1, 1, 1, 1,
               ...
               4, 4, 
               8)
NullProportion2 <- NullRatio2/sum(NullRatio2)
Observed2 <- c(8,14,9,11,8,10,10,7,22,17,25,20,15,18,19,24,14,18,20,16,45,36,38,40,49,48,78) # from Mendel's table
Expected2 <-sum(Observed2)*NullProportion2 # converting proportions to Expected counts
Discrepancy2 <- (Expected2-Observed2)^2/Expected2
TestStat2 <- sum(Discrepancy2) # chi-squared test statistic
p_value2 <- 1-pchisq(TestStat2,27-1) # don't forget the degrees of freedom!
ATable2 <- rbind(NullProportion2,Expected2,Observed2,Discrepancy2)
pander(ATable2, col.names=Category2, digits=3, caption="Table 3: Observed vs. Expected values for Experiment 2")
```

Looking at the table ABc, aBCc, ABbC, AaBC, AaBbC, and AaBbc, all have discrepancies greater than 1. The three categories with highest discrepancies are AaBbC(2.06), AaBbc(1.63), and ABc(1.62).

We take the null distribution to be approximately the chi-squared distribution with degrees of freedom being one minus the number of categories, which in this case is 27-1=26.

The p-value of the sample Mendel took in Experiment 2 is `r round(p_value,3)`. Since p-value = `r round(p_value,3)`> $\alpha$ = 0.1, we fail to reject the null hypothesis because our p-value is too large compared to $\alpha$.
