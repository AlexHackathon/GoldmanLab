...
simulatePhat <- function(n, population){ # this simulates a p hat value
   phat <- mean(sample(population, n, replace=TRUE))
   phat # outputs phat
   }
simulatedPhats <- replicate(1000, # 1000 simulations
          simulatePhat(10,pop)
          )
...
In order to estimate the mean and the SD of the estimated standard error for $\hat p$ when sampling without replacement, we use the same functions as before, changing the replace parameter to FALSE.
...
```
We find that the mean is `r  simulatedPhatsMeanNoRepl` and the standard deviation is about `r  simulatedPhatsSDNoRepl `. To find how much the standard error calculated from a random sample varies from the actual value calculated using the true proportion in the population we use:
...
The distribution of simulated SE have a mean of `r simulatedEstSEsMeanNoRepl` and SD `r simulatedEstSEsSDNoRepl`. The estimated SE is below the SE itself by about `r  sqrt(p*(1-p)/10)-simulatedEstSEsMeanNoRepl` and its spread around the calculated simulated mean is `r simulatedEstSEsSDNoRepl`. Since these are pretty small, we can say that they are a good approximation for the true proportion and standard error. The standard deviation is smaller for the trials without replacement when compared to the ones with replacement.
