...
abstract: \emph{A pdf copy of your assignment is due at 4pm (PT) Wednesday, February 7. Submission of the pdf will be through Gradescope. Please put in the effort to make it look reasonably professional -- you're encouraged to use R Markdown. Note that specific tasks for you are \hl{highlighted}. You are free to work in groups, but you must submit your own writeup, and run your own code.}
header-includes:
...
The code is the same as for the previous case, replacing the reference subsequence with the new values. We then repeat the test 1000 times, using reads of 3000 nucleotides and a total of 12 random reads. The results are displayed below:
```{r}
RefSubseq <- substr(CovSequence, 27894, 28259)

reps1000 <- replicate(1000, Test(CovSequence, RefSubseq, 3000, 12))
resultsHW <- table(reps1000)
resultsHW
```

Using the formula from before, we calculate the sensitivity of the test to be about $\frac{ `r resultsHW[2]` }{  `r resultsHW[1]` + `r resultsHW[2]` } \approx `r round(resultsHW[2]/(resultsHW[1] + resultsHW[2]),2)`$.
