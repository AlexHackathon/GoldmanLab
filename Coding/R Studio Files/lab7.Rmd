...
abstract: \emph{A pdf copy of your assignment is due at 4:00pm PT Wednesday, March 6. Submission of the pdf will be through gradescope. Please put in the effort to make it look reasonably professional -- you're encouraged to use R Markdown. Note that specific tasks for you are \hl{highlighted}. You are free to work in groups, but you must submit your own writeup, and run your own code.}
header-includes:
...
\section*{Fisher's test for association}

\subsection*{Persistence of olfactory/taste alterations from COVID-19 and chronic asthma}
...
\subsection*{Stanford seroprevalence study}
...
[^4]: Stanford's Jay Bhattacharya appears on a political talk show to publicize his claimed results before peer review and the consequent revisions, Fox News, April 14, 2020: https://www.youtube.com/watch?v=6NjCitwKJSQ
...
The null hypothesis for a test for independence is that testing positive and participating in the Stanford study are independent. The alternative hypothesis is that there is an association between testing positive and participating in the Stanford study.

To carry out the test for independence between testing positive and participation in the Stanford study, we perform a Fisher's Exact Test on the contingency table given.
```{r}
fisher.test(SeroprevalenceTable)
```
\vspace{1in}

With a significance level of 0.05, the p-value=.1647 is greater than the significance level, so we fail to reject the null hypothesis that testing positive and participating in the Stanford study are independent. We are unable to disprove that the proportion of false positives is the same in both the manufacturing and Stanford trials. This is inconsistent with Stanford's study because it fails to reject the fact that both of these results could come from the same distribution of false positives, so they can't make their claim based on the data.
